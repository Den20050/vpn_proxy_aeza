...
```json
"log": {
  "level": "warn",       // debug | info | warn | error — warn по умолчанию (меньше шума от сканеров)
  "timestamp": true,     // добавлять метку времени в каждую строку
  "output": "/var/log/sing-box/sing-box.log"
}
```

> **Уровень `warn`**: в лог попадают ошибки и предупреждения, но не каждое входящее подключение. Записи `REALITY: processed invalid connection` от интернет-сканеров при `info` дают десятки тысяч строк в день. Для отладки конкретного клиента временно поставьте `"level": "info"` и выполните `systemctl reload singbox`.
...
**Для РФ (failover по умолчанию)** — в [`config/failover-endpoints.json`](../config/failover-endpoints.json):
...
| server_name | Комментарий |
|-------------|-------------|
| `www.apple.com` | Высокое доверие TLS, но очень узнаваемый SNI |
| `www.amazon.com` | Огромный трафик |
| `www.lovelive-anime.jp` | Популярен в community-конфигах |
...
Текст ниже можно отправить пользователям при жалобах на нестабильное соединение.

> **Если VPN периодически отваливается (~15–20 мин без активности):**
>
> 1. В приложении (Hiddify / v2rayN / Streisand) **отключите Multiplex (mux)**, если включён. С нашим протоколом (`flow=xtls-rprx-vision`) mux **не нужен** и может ухудшать стабильность.
> 2. **Импортируйте `clients/<имя>-singbox.json`** (см. раздел ниже) — автопереключение между несколькими SNI (microsoft + нишевые debian/mozilla/jsdelivr и др.) при таймауте.
> 3. Включите **автопереподключение** / «постоянное соединение».
> 4. На телефоне: **Настройки → Батарея → VPN-приложение → без ограничений** (не «экономия» / не «ограничить в фоне»).
> 5. При обрыве — **переподключитесь**; с failover JSON переключение часто происходит автоматически за ~1 мин.
>
> Если проблема только на мобильной сети, а на Wi‑Fi всё стабильно — это ограничение NAT оператора, а не поломка сервера.

### Автопереключение между SNI (failover)

На одном VPS поднимаются **несколько VLESS+Reality inbound** (по умолчанию **6 SNI**): один популярный primary + нишевые домены с меньшим риском блокировок в РФ.
...
**На сервере** (новый деплой — включено по умолчанию):

```bash
ENABLE_FAILOVER=1 bash scripts/deploy.sh
```

**На уже работающем сервере:**

```bash
cd /opt/vpn_proxy_aeza && git pull
bash scripts/enable-failover.sh          # первый раз
bash scripts/enable-failover.sh --sync   # обновить список SNI
```

Скрипт добавит inbound'ы, откроет порты **8443–8447** в UFW и пересоздаст клиентские конфиги.

**Для пользователей (автопереключение при таймауте):**

Раздайте файл `/root/vpn-backup/clients/<имя>-singbox.json` — внутри **6 outbound** и `urltest` с проверкой каждую **1 минуту**. Клиент выбирает быстрейший SNI; при таймауте/блокировке переключается на другой порт.

```bash
# Сгенерировать/обновить конфиг одного пользователя
bash scripts/generate-client-config.sh user3

# Все пользователи
bash scripts/generate-client-config.sh
```

Импорт: **sing-box** / **Hiddify** → Import from file. Локальный прокси: `socks5://127.0.0.1:2080`.
...
Sing-box пишет прикладные логи в файл; `journalctl` показывает в основном события systemd (старт, reload):

```bash
# Прикладные логи sing-box (основной источник диагностики)
tail -f /var/log/sing-box/sing-box.log
grep "timed out" /var/log/sing-box/sing-box.log | tail -20
grep "invalid connection" /var/log/sing-box/sing-box.log | wc -l   # шум от сканеров

# Системные события сервиса
journalctl -u singbox -n 50 --no-pager
```

**Ротация логов** настраивается в `deploy.sh` → `/etc/logrotate.d/sing-box`:
...
> **Важно:** используется `copytruncate`, а **не** `postrotate → systemctl reload singbox`. Reload при ротации (каждую ночь в ~00:00) обрывал долгоживущие VPN-сессии. `copytruncate` копирует и обнуляет файл без перезагрузки процесса.

Проверить конфиг logrotate:

```bash
logrotate -d /etc/logrotate.d/sing-box   # dry-run, без изменений
...
```bash
# 1. Проверить логи (файл — основной источник; journalctl — события systemd)
tail -100 /var/log/sing-box/sing-box.log
...
### Сценарий 5: Периодические обрывы по таймауту (~15–20 мин)

**Симптом:** VPN работает, но через 15–20 минут без активности (экран выключен, приложение в фоне) соединение рвётся. На сервере в логе:

```
mux connection closed: read frame header: read tcp ...: read: connection timed out
```

Длительность сессии в сообщении об ошибке обычно **16m23s – 16m38s**. IP клиентов часто из мобильных сетей (например, `213.87.x.x` — Мегафон).

**Причина:** не перегрузка сервера, а **idle-таймаут TCP/NAT** на стороне мобильного оператора или mux на клиенте. Сервер при этом `active`, RAM/CPU в норме, другие пользователи подключаются.

**Действия администратора:**

```bash
# 1. Убедиться, что это именно idle-timeout, а не падение сервера
systemctl status singbox
free -h
grep "timed out" /var/log/sing-box/sing-box.log | tail -10

# 2. Проверить, что logrotate не делает reload (см. раздел 10)
grep -E "copytruncate|postrotate" /etc/logrotate.d/sing-box

# 3. Разослать пользователям рекомендации (раздел 6)
```

**Действия пользователя:** импортировать `clients/<имя>-singbox.json` (раздел 6) или отключить mux, включить автопереподключение, снять ограничения батареи.

**Если не помогает:** сменить `server_name` (сценарий 1) или попросить пользователя проверить стабильность на Wi‑Fi vs LTE.
...
# Логи sing-box: таймауты, сканеры, успешные сессии
grep "timed out" /var/log/sing-box/sing-box.log | tail -20
grep "invalid connection" /var/log/sing-box/sing-box.log | wc -l
grep "inbound connection to" /var/log/sing-box/sing-box.log | tail -10
...
# Проверить logrotate (не должно быть postrotate reload)
cat /etc/logrotate.d/sing-box
```

---

*Документ обновлён: версия 1.2, май 2026.*
